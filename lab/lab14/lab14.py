...
def add_trees(t1, t2):
    ...
    for branch1, branch2 in zip(t1.branches, t2.branches):
        n_branches.append(add_trees(branch1, branch2))
    if len(t1.branches) < len(t2.branches):
        n_branches += t2.branches[len(t1.branches):]
    else:
        n_branches += t1.branches[len(t2.branches):]
    return Tree(t1.label+t2.label, n_branches)
