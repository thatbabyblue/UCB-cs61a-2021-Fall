...
class Account:
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            return 'Not enough money left, dickhead!'
        self.balance -= amount
    ...
    def __repr__(self):
        num_deposit, num_withdraw = 0, 0
        for transaction in self.transactions:
            if transaction[0] == 'deposit':
                num_deposit += 1
            else:
                num_withdraw += 1
        return f'Accountholder: {self.holder}, Deposits: {num_deposit}, Withdraws: {num_withdraw}'
...
def shuffle(cards):
    ...
    for i in range(a):
        ...
        shuffled.append(cards[i+a])
    return shuffled

def depth(link):
    if link.rest is Link.empty:
        return 1
    return 1 + depth(link.rest)
...
def longest_length(t):
    if Tree.is_leaf(t):
        return 0
    else:
        return 1 + max([longest_length(b) for b in t.branches])
...
def long_paths(t, n):
    ...
    [[0, 11, 12, 13, 14]]
    """
    if Tree.is_leaf(t) and n <= 0:
        return [[t.label]] 
    else:
        a = []
        for b in t.branches:
            a += insert_into_all(t.label, long_paths(b, n-1))    
        return a
