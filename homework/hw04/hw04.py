...
def sub_interval(x, y):
    ...
    and any value in y."""
    lower = min(lower_bound(x) - lower_bound(y), lower_bound(x) - upper_bound(y))
    upper = max(upper_bound(x) - lower_bound(y), upper_bound(x) - upper_bound(y))
    return interval(lower, upper) 
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0 or upper_bound(y) < 0, 'no interval including 0 is used as a divisor'
