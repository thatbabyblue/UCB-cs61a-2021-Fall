class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return f'{"You must add $"}{str(self.price - self.funds)} {"more funds."}'
        elif self.funds > self.price:
            self.stock -= 1
            self.change = self.funds - self.price
            self.funds = 0
            return f'{"Here is your"} {self.product} {"and $"}{str(self.change)} {"change."}'
        else:
            self.stock -= 1
            self.funds = 0
    ...
    def add_funds(self, amount):
        if self.stock == 0:
            return f'{"Nothing left to vend. Please restock. Here is your $"}{str(amount)}{"."}'
        else:
            self.funds += amount
...
class Coin:
    ...
    def worth(self):
        a = (Mint.present_year - self.year) - 50
        if a > 0:
            return self.cents + a
        else:
            return self.cents
...
def store_digits(n):
    ...
    if n < 10:
        ...
        b = 10**(length(n) - 1)
        n = n % b
        return Link(a//b, store_digits(a%b))
