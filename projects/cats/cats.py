...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k >= len(selected):
        return ''
    else:
        return selected[k]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select_if_contain(paragraph):
        for element in topic:
            if element in split(lower(remove_punctuation(paragraph))):
                return True
        return False
    return select_if_contain
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    elif if_diff(typed_word, valid_words, diff_function, limit):
        return min(valid_words, key=lambda element: diff_function(typed_word, element, limit))
    else:
        return typed_word

def if_diff(typed_word, valid_words, diff_function, limit):
    for element in valid_words:
        if diff_function(typed_word, element, limit) <= limit:
            return True
    return False
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    len_difference = abs(len(start) - len(goal))
    if (len(start) == 0 or len(goal) == 0) and limit >= 0:
...
def minimum_mewtations(start, goal, limit):
    ...
    elif (not start or not goal) and limit >= 0:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    upload({'id': user_id, 'progress': compute_progress(sofar, prompt)})
    return compute_progress(sofar, prompt)

def compute_progress(sofar, prompt):
    i = 0
    while i < len(sofar):
        if sofar[i] != prompt[i]:
            break
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    return match(words, [[times_per_player[x][i+1]-times_per_player[x][i] for i in range(len(times_per_player[x]) - 1)] for x in range(len(times_per_player))])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    return [[word_at(match, x) for x in word_indices if get_times(match).index(min(get_times(match), key=lambda time: time[x])) == i] for i in player_indices]
    # END PROBLEM 10
