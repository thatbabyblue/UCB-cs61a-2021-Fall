...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        homei = self.place
        distance = 0
        while not homei.is_hive:
            if homei.bees and self.min_range <= distance <= self.max_range:
                return random_bee(homei.bees)
            homei = homei.entrance
            distance += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place is None:
            for bee in oriplace.bees[:]:
                Ant.reduce_health(bee, amount + self.damage)
        else:
            for bee in oriplace.bees[:]:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn. A WallAnt is useful because it has a large health value."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt selects a random Bee from its place and eat it whole."""
    name = 'Hungry'
    implemented = True
    food_cost = 4
    chew_duration = 3
    ...
    def action(self, gamestate):
        ...
        elif not self.place.bees:
            return None
        else:
            eat(random_bee(self.place.bees))
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        next = self.place.exit
        while next:
            if next.ant is not None:
                if not next.ant.is_doubled:
                    next.ant.is_doubled = True
                    next.ant.buff()
                if next.ant.is_container and next.ant.ant_contained is not None:
                    if not next.ant.ant_contained.is_doubled:
                        next.ant.ant_contained.is_doubled = True
                        next.ant.ant_contained.buff()
            next = next.exit
